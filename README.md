...
**🔌 Встроенный MCP-сервер (только в десктоп-версии).** Приложение поднимает MCP-сервер прямо внутри себя — твой внешний **Claude Desktop / ChatGPT / Cursor** может полностью управлять Bulka: писать код, запускать и останавливать музыку, читать текущий паттерн. Подключается одним адресом, без установки чего-либо:

```json
{ "mcpServers": { "bulka": { "type": "streamable-http", "url": "http://127.0.0.1:4188/mcp" } } }
```

Управление (старт/стоп/перезапуск) и готовый сниппет — в **Настройках** приложения. Инструменты: `set_code`, `set_code_and_play`, `append_code`, `get_code`, `play`, `stop`.
