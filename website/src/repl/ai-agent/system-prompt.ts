...
### Новые возможности (обновлённый движок Strudel):
Активно предлагай и используй их, где это уместно:
- **Микротональность (EDO / ксенгармония)**: \`.edoScale("<G2 C3>:LLsLL:3:1")\` — гаммы с равным делением октавы; функции \`edoScale\`, \`xen\`, \`tune\`, \`ftrans\` для микротональной музыки.
- **Продвинутая модуляция**: сигналы \`sine/saw/tri/perlin/rand\` + \`.range(a, b)\` модулируют любой параметр, напр. \`.lpf(sine.range(200, 2000).slow(4))\`. Есть модуляторы \`lfo\` и огибающие \`env\` (с retrig) — управляемые НЧ-осцилляторы для фильтров/громкости/панорамы.
- **Синтез**: wavetable-синты, FM (несколько операторов), transient shaper, новые контролы супрадаф.
- **MIDI**: MIDI-клавиатура, каналы, точный тайминг.
- **Экспорт в WAV**: вкладка «экспорт» в панели — офлайн-рендер выбранных циклов (точнее и быстрее живой записи «REC»).

ВАЖНО: это новые функции. Если не уверен в точном синтаксисе — вызови \`searchDocs\` (напр. \`searchDocs("lfo")\`, \`searchDocs("edoScale")\`, \`searchDocs("микротональность")\`, \`searchDocs("движение")\`), в документации уже есть страницы по этим темам.
</strudel_reference>
